...
class wpdb {
	...
	function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Ugyldigt database-pr&#230;fiks'/*/WP_I18N_DB_BAD_PREFIX*/);
	...
	function select( $db, $dbh = null ) {
		...
		if ( !@mysql_select_db( $db, $dbh ) ) {
			$this->ready = false;
			$this->bail( sprintf( /*WP_I18N_DB_SELECT_DB*/'<h1>Kan ikke vælge database</h1>
...
<p>Hvis du ikke ved, hvordan man opretter en database, bør du <strong>kontakte din host</strong>. Hvis alt andet fejler kan du finde hjælp på <a href="http://wordpress.org/support">WordPress Support Forumer</a>, eller på <a href="http://wordpress.dk/forum>WordPress.dks forum</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, htmlspecialchars( $db, ENT_QUOTES ), htmlspecialchars( $this->dbuser, ENT_QUOTES ) ), 'db_select_fail' );
			return;
		}
	}
	...
	function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR_FULL*/'WordPress-databasefejl %1$s for foresp&#248;rgslen %2$s fra %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller );
		else
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR*/'WordPress-databasefejl %1$s for foresp&#248;rgslen %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query );
...
'/*/WP_I18N_DB_CONN_ERROR*/, htmlspecialchars( $this->dbhost, ENT_QUOTES ) ), 'db_connect_fail' );
	...
	function get_row( $query = null, $output = OBJECT, $y = 0 ) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, output type, int offset) -- Output-type skal v&#230;re en af f&#248;lgende: OBJECT, ARRAY_A, ARRAY_N'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
